...
function MushraPage(_pageManager, _audioContext, _bufferSize, _audioFileLoader, _session, _pageConfig, _mushraValidator, _errorHandler, _language, _pageTemplateRenderer) {
  this.isMushra = true;
  ...
  // Isaac: saving how many conditions have been played
  this.conditionsPlayed = [];
  for (var i = 0; i < this.conditions.length + 1; ++i) {
    this.conditionsPlayed[i] = 0;
  }
  // End of edit
...
MushraPage.prototype.init = function () {
  ...
  this.mushraAudioControl.addEventListener((function (_event) {
    if (_event.name == 'stopTriggered') {
      ...
      // Isaac: checking if Next button is unlocked now
      this.conditionsPlayed[index] = 1;
      var unlockNow = this.conditionsPlayed.every(e => e > 0)
      if (unlockNow == true) {
...
MushraPage.prototype.render = function (_parent) {
  ...
  // Isaac: adding pictures here
  if (this.pageConfig.mushraImg != undefined) {
    var mushraImg = $("<img src=\"configs/resources/images/" + this.pageConfig.mushraImg + "\" style='height: auto; width: 70%; object-fit: contain'/>");
    div.append(mushraImg);
  }
  // Isaac: end of edit
...
MushraPage.prototype.load = function () {
  ...
  // Isaac: lock Next button if mustRate == true
